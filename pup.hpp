#ifndef JABC_PUP_HPP
#define JABC_PUP_HPP
//  PUP bindings (DOG-027): an index IS a dog Pup stack — immutable oldest-first
//  runs + a memtable + the 1/8 ladder, all of it in C.  JS holds a handle id
//  plus `dir`/`ext` strings and does nothing but marshalling; the ladder never
//  crosses the boundary.  This INVERTS API.md rule #4 for this subsystem on
//  purpose: the LSM gets exactly ONE home instead of a JS twin of it.
//
//  Leaves (per lane kv64 / wh128 / u64):
//    _pup_<lane>_open(dir, ext, mode)      -> handle (a small integer)
//    _pup_<lane>_put(h, dir, ext, k[, v])  -> undefined  (rw only)
//    _pup_<lane>_commit(h, dir, ext)       -> undefined  (rw only)
//    _pup_<lane>_get(h, k)                 -> val | undefined (newest wins)
//    _pup_<lane>_range(h, lo, hi, cb)      -> undefined  (cb "enough" stops)
//    _pup_<lane>_seek(h, k) / _next(h)     -> merged pull cursor, ONE per handle
//    _pup_<lane>_count(h)                  -> committed run count
//    _pup_<lane>_run(h, i)                 -> Uint8Array over run i (read-only)
//    _pup_<lane>_drop(h, dir, ext[, i])    -> undefined  (no i = every run)
//    _pup_<lane>_close(h)                  -> undefined
//
//  The per-lane `dogpuplane` (sort/dedup/collapse + compaction merge) is
//  instantiated HERE, the way hit.hpp instantiates HIT per lane — dog's Pup is
//  generic, the two typed roles come from the binding.  Each leaf STATICALLY
//  knows its lane and passes that `dogpuplane const *` down as an ordinary C
//  parameter: NO state about the lane is ever dicted, and there is no per-call
//  type either — the lane is fixed on create by which leaf JS calls.
//
//  IMPORTANT: the including TU pulls in hit.hpp first, which instantiates
//  QSORT<lane> (InSort / sDedup / sFindGE) and HIT<lane>* for every lane.
#include "cont.hpp"

extern "C" {
#include "abc/FILE.h"   // FILE_PATH_MAX_LEN
#include "abc/KV.h"     // kv64 (key,val) + kv64Z (KEY-only: a keyed lane)
#include "dog/DOG.h"    // DOGPup* — the LSM itself
#include "dog/WHIFF.h"  // wh128 (key,val) + wh128Z
}

//  DOG-027: open stacks.  The JS handle is an index into this table; the dict
//  behind it is a malloced 64-cell kv64b, freed by _pup_<lane>_close.  EXACTLY
//  64 cells: the memtable's entry is one of them, and no headroom is carved
//  for it — the cap is a damage backstop the 1/8 ladder never approaches, so
//  sizing past it would be sizing for a state already defined as broken.
#define PUP_MAX_OPEN 32
#define PUP_DICT_CELLS HIT_MAX_RUNS

typedef struct {
  Bkv64 pups;
  b8 live;
  b8 rw;
} pupslot;

static pupslot JABC_PUPS[PUP_MAX_OPEN];

//  A run view handed to JS BORROWS the Pup's mapping — drop/close unmaps it,
//  so the view must not outlive the handle (it is the marker-audit path).
static void JABCPupNoFree(void*, void*) {}

//  The ONE plain-words error for a stack past the cap (never a bare C code).
#define PUP_TOOMANY "too many index runs: drop them and re-derive the index"

//  Resolve args[0] to a live slot, or NULL (the caller throws).
static inline pupslot* JABCPupSlot(JSContextRef ctx, size_t argc,
                                   const JSValueRef args[], JSValueRef* ex) {
  if (argc < 1) return NULL;
  u64 h = 0;
  if (!JABCu64Of(&h, ctx, args[0], ex)) return NULL;
  if (h >= PUP_MAX_OPEN || !JABC_PUPS[h].live) return NULL;
  return &JABC_PUPS[h];
}

//  The lane element needles: pair lanes take ONE BigInt (the key, with the val
//  field at its low bound 0), u64 the scalar itself.
static inline kv64 JABCPupKv64(JSContextRef ctx, const JSValueRef args[],
                               size_t i, JSValueRef* ex) {
  kv64 v;
  v.key = (u64)JSValueToUInt64(ctx, args[i], ex);
  v.val = 0;
  return v;
}
static inline wh128 JABCPupWh128(JSContextRef ctx, const JSValueRef args[],
                                 size_t i, JSValueRef* ex) {
  wh128 v;
  v.key = (u64)JSValueToUInt64(ctx, args[i], ex);
  v.val = 0;
  return v;
}
static inline u64 JABCPupU64(JSContextRef ctx, const JSValueRef args[],
                             size_t i, JSValueRef* ex) {
  return (u64)JSValueToUInt64(ctx, args[i], ex);
}

//  cb's return is a stop signal: false / "enough" stops, anything else goes on
//  (the io.readdir(path, cb) contract).
static inline bool JABCPupEnough(JSContextRef ctx, JSValueRef r) {
  if (!JSValueIsUndefined(ctx, r) && JSValueIsBoolean(ctx, r) &&
      !JSValueToBoolean(ctx, r))
    return true;
  if (JSValueIsString(ctx, r)) {
    char tag[8] = "";
    JSStringRef s = JSValueToStringCopy(ctx, r, NULL);
    JSStringGetUTF8CString(s, tag, sizeof(tag));
    JSStringRelease(s);
    return strcmp(tag, "enough") == 0;
  }
  return false;
}

//  Read `dir` and `ext` (plain JS strings on the handle) into path buffers.
#define PUP_PATHS(A)                                            \
  a_pad(u8, dirp, FILE_PATH_MAX_LEN);                           \
  a_pad(u8, extp, FILE_PATH_MAX_LEN);                           \
  if (JABCPath(dirp, ctx, args[A], exception) != OK ||          \
      JABCPath(extp, ctx, args[(A) + 1], exception) != OK)      \
  JABC_THROW("the index dir and ext must be strings")

//  --- per-lane leaves -------------------------------------------------------
//
//  L      lane element type (kv64 / wh128 / u64)
//  RD     needle reader (ctx, args, i, exception) -> L
//  PUTVAL fills the record's val field from args[4] (pair lanes only)
//  EMIT   JS value for the merged element `top`
//  GETV   JS value _get returns for the matching element `pos`
//  KEYEQ  does `pos` carry `needle`'s key?
#define PUP_LEAVES(L, RD, PUTVAL, EMIT, GETV, KEYEQ)                           \
  /*  the lane's typed sort/dedup/collapse: STABLE insertion sort + keep-last  \
      dedup on DATA; on collapse, fold PAST+DATA with the ties going to DATA */\
  static ok64 jpup_sync_##L(u8bp mem, b8 collapse) {                           \
    sane(u8bOK(mem));                                                          \
    L* dh = (L*)mem[1];                                                        \
    L* de = (L*)mem[2];                                                        \
    b8 clean = YES;                                                            \
    for (L* p = dh + 1; p < de; p++)                                           \
      if (!L##Z(p - 1, p)) {                                                   \
        clean = NO;                                                            \
        break;                                                                 \
      }                                                                        \
    if (!clean) {                                                              \
      QSORT##L##InSort(dh, de);                                                \
      L##s d = {dh, de};                                                       \
      L##sDedup(d);                                                            \
      call(u8bShed, mem, (size_t)(de - d[1]) * sizeof(L));                     \
    }                                                                          \
    if (!collapse) done;                                                       \
    if (mem[0] == mem[1]) {                                                    \
      call(u8bUsedAll, mem);                                                   \
      done;                                                                    \
    }                                                                          \
    a_carve(L, scr, DOG_PUP_MEM_BYTES / sizeof(L));                            \
    L##cs runs[2] = {{(L const*)mem[0], (L const*)mem[1]},                     \
                     {(L const*)mem[1], (L const*)mem[2]}};                    \
    L##css hp = {runs, runs + 2};                                              \
    L* base = L##bIdleHead(scr);                                               \
    L##s into = {base, L##bTerm(scr)};                                         \
    call(HIT##L##Merge, hp, into);                                             \
    u8cs merged = {(u8c*)base, (u8c*)into[0]};                                 \
    u8bReset(mem);                                                             \
    call(u8bFeed, mem, merged);                                                \
    call(u8bUsedAll, mem);                                                     \
    done;                                                                      \
  }                                                                            \
  /*  the lane's typed compaction merge — equal keys go to the youngest run */ \
  static ok64 jpup_merge_##L(u8s into, u8css srcs) {                           \
    sane($ok(into));                                                           \
    size_t n = (size_t)$len(srcs);                                             \
    if (n > HIT_MAX_RUNS) return HITTOOMANY;                                   \
    L##cs runs[HIT_MAX_RUNS];                                                  \
    for (size_t i = 0; i < n; i++) {                                           \
      runs[i][0] = (L const*)srcs[0][i][0];                                    \
      runs[i][1] = (L const*)srcs[0][i][1];                                    \
    }                                                                          \
    L##css hp = {runs, runs + n};                                              \
    L* base = (L*)into[0];                                                     \
    L##s dst = {base, (L*)into[1]};                                            \
    call(HIT##L##Merge, hp, dst);                                              \
    into[0] = (u8*)dst[0];                                                     \
    done;                                                                      \
  }                                                                            \
  static dogpuplane const JABC_PUP_LANE_##L = {jpup_sync_##L, jpup_merge_##L}; \
  /*  the handle's ONE merged cursor: the entries advance in place, so each    \
      _next re-Loads the pointer heap over them (HITSkipValue's shape). */     \
  static L##cs JABC_PUPCUR_##L[PUP_MAX_OPEN][HIT_MAX_RUNS];                    \
  static size_t JABC_PUPCURN_##L[PUP_MAX_OPEN];                                \
                                                                               \
  /*  the query sources: committed runs oldest->newest, then the memtable's    \
      PAST and DATA — 1 or 2 more HIT runs, never a special-cased path. */     \
  static ok64 jpup_src_##L(L##cs* ent, size_t* n, kv64b pups) {                \
    sane(ent != NULL && n != NULL);                                            \
    *n = 0;                                                                    \
    Bu8cs srcs = {};                                                           \
    call(u8csbAllocate, srcs, HIT_MAX_RUNS);                                   \
    try(DOGPupAllRuns, srcs, pups, &JABC_PUP_LANE_##L);                        \
    nedo {                                                                     \
      u8csbFree(srcs);                                                         \
      return HITTOOMANY;                                                       \
    }                                                                          \
    size_t k = (size_t)u8csbDataLen(srcs);                                     \
    u8cs* sl = u8csbDataHead(srcs);                                            \
    for (size_t i = 0; i < k; i++) {                                           \
      ent[i][0] = (L const*)sl[i][0];                                          \
      ent[i][1] = (L const*)sl[i][1];                                          \
    }                                                                          \
    *n = k;                                                                    \
    u8csbFree(srcs);                                                           \
    done;                                                                      \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_open) {                                            \
    if (argc < 2) JABC_THROW("_pup_" #L "_open(dir, ext, mode)");              \
    PUP_PATHS(0);                                                              \
    b8 rw = YES;                                                               \
    if (argc >= 3 && JSValueIsString(ctx, args[2])) {                          \
      char mode[8] = "";                                                       \
      JSStringRef ms = JSValueToStringCopy(ctx, args[2], NULL);                \
      JSStringGetUTF8CString(ms, mode, sizeof(mode));                          \
      JSStringRelease(ms);                                                     \
      rw = (b8)(strcmp(mode, "r") != 0);                                       \
    }                                                                          \
    int h = -1;                                                                \
    for (int i = 0; i < PUP_MAX_OPEN; i++)                                     \
      if (!JABC_PUPS[i].live) {                                                \
        h = i;                                                                 \
        break;                                                                 \
      }                                                                        \
    if (h < 0) JABC_THROW("too many open indexes");                            \
    pupslot* s = &JABC_PUPS[h];                                                \
    zero(s->pups);                                                             \
    if (kv64bAllocate(s->pups, PUP_DICT_CELLS) != OK)                          \
      JABC_THROW("cannot allocate the index");                                 \
    /*  the dict IS the cap, so a dir that overflows it is exactly a stack     \
        past 64 runs — damaged or foreign, and the cure is to re-derive. */    \
    if (DOGPupOpenAll(s->pups, $path(dirp), $path(extp)) != OK ||              \
        DOGPupCount(s->pups) > HIT_MAX_RUNS) {                                 \
      DOGPupClose(s->pups);                                                    \
      JABC_THROW(PUP_TOOMANY);                                                 \
    }                                                                          \
    s->live = YES;                                                             \
    s->rw = rw;                                                                \
    JABC_PUPCURN_##L[h] = 0;                                                   \
    return JSValueMakeNumber(ctx, (double)h);                                  \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_put) {                                             \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 4) JABC_THROW("_pup_" #L "_put(h, dir, ext, k[, v])");    \
    if (!s->rw) JABC_THROW("this index is open read-only");                    \
    PUP_PATHS(1);                                                              \
    L row = RD(ctx, args, 3, exception);                                       \
    PUTVAL;                                                                    \
    if (*exception) JABC_UNDEF;                                                \
    u8cs rec = {(u8c*)&row, (u8c*)(&row + 1)};                                 \
    ok64 o = DOGPupPut(s->pups, $path(dirp), $path(extp), rec,                 \
                       &JABC_PUP_LANE_##L);                                    \
    if (o == HITTOOMANY) JABC_THROW(PUP_TOOMANY);                              \
    if (o != OK) JABC_THROW("cannot write to the index");                      \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_commit) {                                          \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 3) JABC_THROW("_pup_" #L "_commit(h, dir, ext)");         \
    if (!s->rw) JABC_THROW("this index is open read-only");                    \
    PUP_PATHS(1);                                                              \
    ok64 o = DOGPupCommit(s->pups, $path(dirp), $path(extp),                   \
                          &JABC_PUP_LANE_##L);                                 \
    if (o == HITTOOMANY) JABC_THROW(PUP_TOOMANY);                              \
    if (o != OK) JABC_THROW("cannot commit the index");                        \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  /*  point lookup: newest source first (they arrive oldest-first, so walk     \
      from the END), FindGE the key, accept iff the key matches. */            \
  static JABC_FN(jpup_##L##_get) {                                             \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 2) JABC_THROW("_pup_" #L "_get(h, k)");                   \
    L needle = RD(ctx, args, 1, exception);                                    \
    if (*exception) JABC_UNDEF;                                                \
    L##cs ent[HIT_MAX_RUNS];                                                   \
    size_t n = 0;                                                              \
    if (jpup_src_##L(ent, &n, s->pups) != OK) JABC_THROW(PUP_TOOMANY);         \
    for (size_t j = n; j > 0; j--) {                                           \
      L const* pos = L##sFindGE(ent[j - 1], &needle);                          \
      if (pos < ent[j - 1][1] && (KEYEQ)) return (GETV);                       \
    }                                                                          \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  /*  ordered [lo, hi) scan: trim every source to the range, then drain the    \
      pointer heap newest-wins (Tops/AdvanceTops collapse an equal-key group   \
      onto its youngest member), streaming through the in-frame cb. */         \
  static JABC_FN(jpup_##L##_range) {                                           \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 4) JABC_THROW("_pup_" #L "_range(h, lo, hi, cb)");        \
    L lo = RD(ctx, args, 1, exception), hi = RD(ctx, args, 2, exception);      \
    if (*exception) JABC_UNDEF;                                                \
    if (!JSValueIsObject(ctx, args[3]) ||                                      \
        !JSObjectIsFunction(ctx, (JSObjectRef)args[3]))                        \
      JABC_THROW("the range callback must be a function");                     \
    JSObjectRef cb = (JSObjectRef)args[3];                                     \
    L##cs ent[HIT_MAX_RUNS];                                                   \
    size_t n = 0;                                                              \
    if (jpup_src_##L(ent, &n, s->pups) != OK) JABC_THROW(PUP_TOOMANY);         \
    L##css heap = {ent, ent + n};                                              \
    HIT##L##SeekRange(heap, &lo, &hi);                                         \
    L##csp slots[HIT_MAX_RUNS];                                                \
    L##csps ph;                                                                \
    if (HIT##L##Load(ph, slots, heap) != OK) JABC_THROW(PUP_TOOMANY);          \
    while (!$empty(ph)) {                                                      \
      L const* top = (*ph[0])[0];                                              \
      L val = *top;                                                            \
      JSValueRef el = EMIT;                                                    \
      size_t ntops = HIT##L##Tops(ph);                                         \
      HIT##L##AdvanceTops(ph, ntops);                                          \
      while (!$empty(ph) && !L##Z((*ph[0])[0], &val) &&                        \
             !L##Z(&val, (*ph[0])[0]))                                         \
        HIT##L##Step(ph);                                                      \
      JSValueRef exc = NULL;                                                   \
      JSValueRef r = JSObjectCallAsFunction(ctx, cb, NULL, 1, &el, &exc);      \
      if (exc != NULL) {                                                       \
        *exception = exc;                                                      \
        JABC_UNDEF;                                                            \
      }                                                                        \
      if (JABCPupEnough(ctx, r)) break;                                        \
    }                                                                          \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  /*  seek: position every source at the first element >= k and keep those     \
      entries on the handle; _next pulls ONE merged row per call. */           \
  static JABC_FN(jpup_##L##_seek) {                                            \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 2) JABC_THROW("_pup_" #L "_seek(h, k)");                  \
    size_t h = (size_t)(s - JABC_PUPS);                                        \
    L needle = RD(ctx, args, 1, exception);                                    \
    if (*exception) JABC_UNDEF;                                                \
    L##cs* ent = JABC_PUPCUR_##L[h];                                           \
    size_t n = 0;                                                              \
    if (jpup_src_##L(ent, &n, s->pups) != OK) JABC_THROW(PUP_TOOMANY);         \
    size_t w = 0;                                                              \
    for (size_t i = 0; i < n; i++) {                                           \
      L const* pos = L##sFindGE(ent[i], &needle);                              \
      if (pos >= ent[i][1]) continue;                                          \
      ent[w][0] = pos;                                                         \
      ent[w][1] = ent[i][1];                                                   \
      w++;                                                                     \
    }                                                                          \
    JABC_PUPCURN_##L[h] = w;                                                   \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_next) {                                            \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s) JABC_THROW("_pup_" #L "_next(h)");                                 \
    size_t h = (size_t)(s - JABC_PUPS);                                        \
    L##cs* ent = JABC_PUPCUR_##L[h];                                           \
    L##css runs = {ent, ent + JABC_PUPCURN_##L[h]};                            \
    L##csp slots[HIT_MAX_RUNS];                                                \
    L##csps ph;                                                                \
    if (HIT##L##Load(ph, slots, runs) != OK) JABC_THROW(PUP_TOOMANY);          \
    JABC_PUPCURN_##L[h] = (size_t)$len(runs); /* Load compacts the entries */  \
    if ($empty(ph)) JABC_UNDEF;                                                \
    L const* top = (*ph[0])[0];                                                \
    L val = *top;                                                              \
    JSValueRef el = EMIT;                                                      \
    size_t ntops = HIT##L##Tops(ph);                                           \
    HIT##L##AdvanceTops(ph, ntops);                                            \
    while (!$empty(ph) && !L##Z((*ph[0])[0], &val) && !L##Z(&val, (*ph[0])[0]))\
      HIT##L##Step(ph);                                                        \
    return el;                                                                 \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_count) {                                           \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s) JABC_THROW("_pup_" #L "_count(h)");                                \
    return JSValueMakeNumber(ctx, (double)DOGPupCount(s->pups));               \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_run) {                                             \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 2) JABC_THROW("_pup_" #L "_run(h, i)");                   \
    u64 i = 0;                                                                 \
    if (!JABCu64Of(&i, ctx, args[1], exception)) JABC_UNDEF;                   \
    if (i >= DOGPupCount(s->pups)) JABC_THROW("no such index run");            \
    u8cs run = {};                                                             \
    DOGPupData(run, s->pups, (u32)i);                                          \
    if (run[0] == NULL) JABC_THROW("no such index run");                       \
    return JSObjectMakeTypedArrayWithBytesNoCopy(                              \
        ctx, kJSTypedArrayTypeUint8Array, (void*)(uintptr_t)run[0],            \
        (size_t)u8csLen(run), JABCPupNoFree, NULL, exception);                 \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_drop) {                                            \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s || argc < 3) JABC_THROW("_pup_" #L "_drop(h, dir, ext[, i])");      \
    if (!s->rw) JABC_THROW("this index is open read-only");                    \
    PUP_PATHS(1);                                                              \
    JABC_PUPCURN_##L[(size_t)(s - JABC_PUPS)] = 0; /* the cursor's runs go */  \
    if (argc < 4 || JSValueIsUndefined(ctx, args[3])) {                        \
      /*  drop() = every run: the family found no marker and re-derives */     \
      if (DOGPupThinTail(s->pups, $path(dirp), $path(extp),                    \
                         DOGPupCount(s->pups)) != OK)                          \
        JABC_THROW("cannot drop the index runs");                              \
      JABC_UNDEF;                                                              \
    }                                                                          \
    u64 i = 0;                                                                 \
    if (!JABCu64Of(&i, ctx, args[3], exception)) JABC_UNDEF;                   \
    if (DOGPupDropAt(s->pups, $path(dirp), $path(extp), (u32)i) != OK)         \
      JABC_THROW("no such index run");                                         \
    JABC_UNDEF;                                                                \
  }                                                                            \
                                                                               \
  static JABC_FN(jpup_##L##_close) {                                           \
    pupslot* s = JABCPupSlot(ctx, argc, args, exception);                      \
    if (!s) JABC_THROW("_pup_" #L "_close(h)");                                \
    size_t h = (size_t)(s - JABC_PUPS);                                        \
    DOGPupClose(s->pups);                                                      \
    zero(s->pups);                                                             \
    s->live = NO;                                                              \
    JABC_PUPCURN_##L[h] = 0;                                                   \
    JABC_UNDEF;                                                                \
  }

PUP_LEAVES(kv64, JABCPupKv64,
           (row.val = (u64)JSValueToUInt64(ctx, args[4], exception)),
           JABCPair(ctx, JSBigIntCreateWithUInt64(ctx, top->key, exception),
                    JSBigIntCreateWithUInt64(ctx, top->val, exception)),
           JSBigIntCreateWithUInt64(ctx, pos->val, exception),
           pos->key == needle.key)

PUP_LEAVES(wh128, JABCPupWh128,
           (row.val = (u64)JSValueToUInt64(ctx, args[4], exception)),
           JABCPair(ctx, JSBigIntCreateWithUInt64(ctx, top->key, exception),
                    JSBigIntCreateWithUInt64(ctx, top->val, exception)),
           JSBigIntCreateWithUInt64(ctx, pos->val, exception),
           pos->key == needle.key)

PUP_LEAVES(u64, JABCPupU64, ((void)0),
           JSBigIntCreateWithUInt64(ctx, *top, exception),
           JSBigIntCreateWithUInt64(ctx, *pos, exception), *pos == needle)

#define PUP_REG(L)                                       \
  JABC_API_FN(o, "_pup_" #L "_open", jpup_##L##_open);   \
  JABC_API_FN(o, "_pup_" #L "_put", jpup_##L##_put);     \
  JABC_API_FN(o, "_pup_" #L "_commit", jpup_##L##_commit); \
  JABC_API_FN(o, "_pup_" #L "_get", jpup_##L##_get);     \
  JABC_API_FN(o, "_pup_" #L "_range", jpup_##L##_range); \
  JABC_API_FN(o, "_pup_" #L "_seek", jpup_##L##_seek);   \
  JABC_API_FN(o, "_pup_" #L "_next", jpup_##L##_next);   \
  JABC_API_FN(o, "_pup_" #L "_count", jpup_##L##_count); \
  JABC_API_FN(o, "_pup_" #L "_run", jpup_##L##_run);     \
  JABC_API_FN(o, "_pup_" #L "_drop", jpup_##L##_drop);   \
  JABC_API_FN(o, "_pup_" #L "_close", jpup_##L##_close)

static inline void JABCPupInstall(JSObjectRef o) {
  PUP_REG(kv64);
  PUP_REG(wh128);
  PUP_REG(u64);
}

#endif
